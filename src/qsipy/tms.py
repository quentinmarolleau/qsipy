...
def derivative_phase_resolution_difference_finite_qe(
    ...
    estimation during an interferometry experiment using two-mode squeezed states,
    detectors with finite quantum efficiency and considering the variance of the
    difference of particles at the output as the observable of interest. The detectors
    have a finite quantum efficiency eta.
    ...
    nu = N / 2
    return (
        -8 * eta * (1 + nu) * (1 + 8 * eta * nu * (1 + eta * nu))
        - (-1 + eta) * (-1 + 10 * (-1 + eta) * eta * nu) * (1 / np.sin(phi) ** 2)
        + (
            1
            + eta
            * (7 + 2 * nu * (9 + eta * (22 + 32 * nu + eta * (5 + 32 * nu * (1 + nu)))))
        )
        * (1 / np.cos(phi) ** 2)
    ) / (
        4
